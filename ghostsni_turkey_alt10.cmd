@echo off
title GhostSNI - Modeset 2 - Dengeli (frag + HTTP tricks)
...
if exist "%~dp0bin\GhostSNI.exe" (
    start "" "%~dp0bin\GhostSNI.exe" -2
    goto :started
)
if exist "%~dp0build\GhostSNI.exe" (
    start "" "%~dp0build\GhostSNI.exe" -2
    goto :started
)
if exist "%~dp0GhostSNI.exe" (
    start "" "%~dp0GhostSNI.exe" -2
    goto :started
)
...
echo  Profil: Modeset 2 - Dengeli (frag + HTTP tricks)
echo.
