@echo off
title GhostSNI - Modeset 1 - En uyumlu (sadece HTTP tricks)
...
if exist "%~dp0bin\GhostSNI.exe" (
    start "" "%~dp0bin\GhostSNI.exe" -1
    goto :started
)
if exist "%~dp0build\GhostSNI.exe" (
    start "" "%~dp0build\GhostSNI.exe" -1
    goto :started
)
if exist "%~dp0GhostSNI.exe" (
    start "" "%~dp0GhostSNI.exe" -1
    goto :started
)
...
echo  Profil: Modeset 1 - En uyumlu (sadece HTTP tricks)
echo.
