@echo off
title GhostSNI - Frag + fake (passive DPI kapali)
...
if exist "%~dp0bin\GhostSNI.exe" (
    start "" "%~dp0bin\GhostSNI.exe" -f 2 -e -b -r -s -m
    goto :started
)
if exist "%~dp0build\GhostSNI.exe" (
    start "" "%~dp0build\GhostSNI.exe" -f 2 -e -b -r -s -m
    goto :started
)
if exist "%~dp0GhostSNI.exe" (
    start "" "%~dp0GhostSNI.exe" -f 2 -e -b -r -s -m
    goto :started
)
...
echo  Profil: Frag + fake (passive DPI kapali)
echo.
